...
from .error import SchemaMissingKeyError, SchemaUnexpectedTypeError, DataUnmatchedTypeError
...
class Validator:
    ...
    @staticmethod
    def check_structure(schema, data, error=None):

        if Validator._is_dict(schema, data):
            for key in schema:
                if key not in data:
                    raise SchemaMissingKeyError("Missing key: %s" % key)
                Validator.check_structure(schema[key], data[key], error)

        elif Validator._is_list(schema, data):
            # schema  is list in the form [type or dict]
            for datum in data:
                Validator.check_structure(schema[0], datum, error)

        elif Validator._is_type(schema):
            # structure is the type of data
            if not Validator._is_data_match_type(schema, data):
                raise DataUnmatchedTypeError(
                    "Mismatch data with type: '%s' is not instance of %s" % (data, schema), error)

        else:
            # structure is neither a dict, nor list, not TYPES
            raise SchemaUnexpectedTypeError("Unrecognizable type in schema: %s" % schema, error)
